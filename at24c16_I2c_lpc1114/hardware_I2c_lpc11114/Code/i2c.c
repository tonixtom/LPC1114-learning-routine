...
/****************************************/
/* 函数功能：初始化LPC1114 LPC_I2C模块      */
/* 入口参数：Mode :0 , 慢速模式         */
/*                 1 , 快速模式         */
/****************************************/
void I2C_Init(uint8_t Mode)
{
	LPC_SYSCON->PRESETCTRL |= (1<<1); // De-asserted I2C模块（在启动LPC_I2C模块之前，必须向该位写1）
 	LPC_SYSCON->SYSAHBCLKCTRL |= (1<<5); // 使能LPC_I2C时钟
	LPC_SYSCON->SYSAHBCLKCTRL |= (1<<16); // 使能LPC_IOCON时钟
  	LPC_IOCON->PIO0_4 &= ~0x3F;	    
  	LPC_IOCON->PIO0_4 |= 0x01;		// 把P0.4脚配置为 I2C SCL 
  	LPC_IOCON->PIO0_5 &= ~0x3F;	
  	LPC_IOCON->PIO0_5 |= 0x01;		// 把P0.5脚配置为 I2C SDA 
	LPC_SYSCON->SYSAHBCLKCTRL |= (1<<16); // 禁能LPC_IOCON时钟 
   	if(Mode == 1)	// 快速I2C通信 (大约400KHz传输速率)（AT24C16支持400K快速模式） 速率为50Mhz/500 = 100kb/s
   	{
      	LPC_I2C->SCLH = 47;  // 0.8us
     	LPC_I2C->SCLL = 93;  // 1.4us
   	}
   	else 	// 低速LPC_I2C通信	 (大约100KHz传输速率)   速递为50Mhz/500 = 100kb/s
...
uint8_t I2C_Send_Ctrl(uint8_t CtrlAndAddr)
{
	...
	// 发送命令+地址字节
   	...
   	{
      	I2C_Stop();		 // 没有完成任务，发送停止信号，结束LPC_I2C通信
      	return 1;			 // 返回1，表明失败！
   	}
   	return 0;			 // 如果正确执行返回0
}
